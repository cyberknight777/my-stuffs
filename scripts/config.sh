...
check_host() {
	...
	if [ "$host" == '"(kali)"' ]; then
		...
		wrap_bad "- CONFIG_DEFAULT_HOSTNAME" "missing $(
			echo -ne '\033['"93"'m'
...
echo -en '\033['""'m'
wrap_color "READING CONFIG FROM: $(
	echo -ne '\033['"1"'m'
	echo -ne '\033['"36"'m'
...
if [ "$CONFIG" == "/proc/config.gz" ]; then
	...
	echo -ne ":$(
		...
		echo -ne "$CONFIG" | tr '[:lower:]' '[:upper:]'
		echo -en '\033['""'m'
	) MAY NOT GIVE PROPER CONFIG RESULT"
	echo -en '\033['""'m'
fi
...
if [ "$MSA" == 'y' ]; then
	wrap_bad "- CONFIG_MEDIA_SUBDRV_AUTOSELECT" "enabled $(
		echo -ne '\033['"93"'m'
		echo -ne "(Disable Wen!!)"
		echo -en '\033['""'m'
	)"
else
	wrap_bad "- CONFIG_MEDIA_SUBDRV_AUTOSELECT" "missing $(
		echo -ne '\033['"32"'m'
		echo -ne "(Good Sign!!)"
		echo -en '\033['""'m'
	)"
